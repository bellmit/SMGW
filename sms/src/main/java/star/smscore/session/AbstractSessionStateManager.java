...
import star.smscore.common.SendFailException;
import star.smscore.common.SmsLifeTerminateException;
import star.smscore.common.storedMap.VersionObject;
import star.smscore.common.util.CachedMillisecondClock;
...
public abstract class AbstractSessionStateManager<K, T extends BaseMessage> extends ChannelDuplexHandler {
	 ...
	 */
	private final ConcurrentMap<K, VersionObject<T>> storeMap = new ConcurrentHashMap<K, VersionObject<T>>();
...
//		Entry entry = msgRetryMap.remove(getSequenceId(requestMsg));
	...
	private Promise<T> safewrite(final ChannelHandlerContext ctx, final T message, final ChannelPromise promise,boolean syn) {
		if (ctx.channel().isActive()) {
			...
			final K seq = getSequenceId(message);
			errlogger.info("发送消息了"+seq.toString());
			...
			msgWriteCount++;
			// 持久化到队列
			VersionObject<T> obj = new VersionObject<T>(message);
			storeMap.put(seq, obj);
//			errlogger.info("发送消息了，storeMap:{}",storeMap);
//			errlogger.info("发送消息了，hasv:{}",storeMap.get(seq)==null?"1":"0");
//			errlogger.info("发送消息了，v:{}",storeMap.get(seq));
//			errlogger.info("发送消息了，message:{}",storeMap.get(seq).getObj());
